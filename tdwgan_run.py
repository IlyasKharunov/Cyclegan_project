...
opt = Params(startepoch = 0, n_epochs = 200, 
             batchSize = 60, dataroot = 'database', 
...
for epoch in range(opt.startepoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        for _ in range(1):
            ...
            x_tilde=eps*real_A+(1-eps)*real_B
        ...
        for _ in range(1):
            ...
            # Fake loss
            pred_fake = netD_B(real_A)
            # fake loss part of wgan loss
            loss_D_fake = torch.mean(pred_fake)

            eps = torch.rand((1),requires_grad=True, device = 'cuda')
            eps = eps.expand(real_B.size())
            x_tilde=eps*real_B+(1-eps)*real_A
    ...
    torch.save(netD_A.state_dict(), f'{base}outputwgp/twnetD_A{epoch%2}.pth')
    torch.save(netD_B.state_dict(), f'{base}outputwgp/twnetD_B{epoch%2}.pth')
###################################
