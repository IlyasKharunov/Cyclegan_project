...
for epoch in range(opt.startepoch, opt.n_epochs):
    ...
    torch.save(netG_A2B.state_dict(), f'{base}output/netG_A2B{epoch}.pth')
    torch.save(netG_B2A.state_dict(), f'{base}output/netG_B2A{epoch}.pth')
    torch.save(netD_A.state_dict(), f'{base}output/netD_A{epoch}.pth')
    torch.save(netD_B.state_dict(), f'{base}output/netD_B{epoch}.pth')
###################################
